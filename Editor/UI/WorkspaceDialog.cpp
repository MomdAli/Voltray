...
namespace Editor::UI
{
    ...
    std::vector<Workspace> WorkspaceDialog::s_Workspaces;
...
// Set default path to user's Documents folder
        ...
        if (ImGui::Button("Open Workspace"))
        {
            ...
            {
                OnWorkspaceSelected(&s_Workspaces[s_SelectedWorkspace]);
            }
        }
        ImGui::EndDisabled();
    }
    ...
    {
        s_Workspaces = WorkspaceManager::GetAllWorkspaces();
    ...
    void WorkspaceDialog::CreateWorkspace()
    {
        ...
        if (WorkspaceManager::CreateWorkspace(s_NewWorkspaceName, fullPath, s_NewWorkspaceDescription))
        {
            ...
            for (int i = 0; i < static_cast<int>(s_Workspaces.size()); ++i)
            {
                ...
                {
                    OnWorkspaceSelected(&s_Workspaces[i]);
                    return;
                }
            }
        }
    ...
    void WorkspaceDialog::OnWorkspaceSelected(const Workspace *workspace)
    {
        if (s_Callback)
        {
