...
void Mesh::GetBounds(Vec3 &minBounds, Vec3 &maxBounds) const
{
    ...
    m_MinBounds = Vec3(m_Vertices[0], m_Vertices[1], m_Vertices[2]);
    ...
    for (size_t i = 0; i < m_Vertices.size(); i += stride)
    {
        ...
        {
            Vec3 position(m_Vertices[i], m_Vertices[i + 1], m_Vertices[i + 2]);
